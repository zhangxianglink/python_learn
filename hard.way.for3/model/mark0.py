...
output_dir = st.text_input("输入保存文件的目录", value="/Users/cool/Desktop/Voicenewcontrast2/user")
            ...
            for i, mark in enumerate(flat_output_data):
                with st.expander(f"**标注 {i + 1}**:"):
                    ...
                    if show_subtitle:
                        ...
                        mark['子选项原文摘要'] = st.text_area(f"子选项原文摘要 {i + 1}", value=mark.get('子选项原文摘要', ''), key=f"subtitle_summary_{i}")  # 如果不存在，则默认为空字符串
                        mark['子选项解释'] = st.text_area(f"子选项解释 {i + 1}", value=mark.get('子选项解释', ''), key=f"subtitle_explanation_{i}")  # 如果不存在，则默认为空字符串
                    else:
                        mark.pop('子选项', None)
