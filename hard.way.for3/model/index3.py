...
titles = list(titles_with_numbers.keys())
...
if 'data' in st.session_state:
    ...
    if st.session_state.current_index < total_records:
        ...
        with col1:
            ...
            for mark in output_data:
                if mark["原文摘要"] in current_record['input']:
                    highlighted_text = f'<span style="background-color: yellow;">{mark["原文摘要"]}</span>'
                else:
                    highlighted_text = f'<span style="background-color: red;">{mark["原文摘要"]}</span>'
                info_html = info_html.replace(mark["原文摘要"], highlighted_text)
        ...
        with col2:
            ...
            for i, mark in enumerate(output_data):
                ...
                selected_title = st.selectbox(f"标题 {i + 1}", titles, index=titles.index(mark['标题']), key=f"title_{i}")
                mark['标题'] = selected_title
                mark['编号'] = titles_with_numbers[selected_title]  # 根据标题更新编号
